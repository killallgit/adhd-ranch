...
- `task check:windows` type-checks the workspace for a Windows target from any OS — the only local way to catch a `#[cfg(unix)]` symbol used without a gate, which otherwise surfaces in `windows-build-smoke.yml` minutes after a push. Needs `rustup target add x86_64-pc-windows-msvc` and `brew install llvm`.
